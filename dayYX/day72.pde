...
void setup() {
  ...
  img = createImage(512, fCount, RGB); // fft spec size x seconds
  img.loadPixels();
}
...
void draw() {
  ...
  for(int i = 0; i < fft.specSize(); i++)
  {
    ...
    //h = log(h) * 200;
    //img.pixels[(frameCount-1)*512+i] = color(h,h,h);
  ...
  //  x = sin(angle * freq0/freqDiv) * width/2.0;
  //  y = sin(angle * freq1/freqDiv) * height/2.0;
